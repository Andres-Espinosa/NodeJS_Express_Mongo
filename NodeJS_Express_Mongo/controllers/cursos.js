...
const { cursoSchemaValidation } = require('../validaciones/cursos_validations'); // Importa la validación
    ...
    for (let curso of cursos) {
        ...
        if (error) {
            return res.status(400).json({
                error: `Error en curso "${curso.titulo}":
${error.details[0].message}`
    ...
    try {
        ...
        res.status(500).json({
            error: 'Error interno del servidor al guardar cursos', details:
                err.message
        });
    }
};
    ...
    try {
        ...
        if (!usuarios || usuarios.length === 0) {
            return res.status(404).json({
                error: `No se encontraron usuarios para el curso con ID
    ${id}`
            });
        }
        res.json(usuarios);
        ...
        res.status(500).json({
            error: `Error interno del servidor al buscar los usuarios del curso:
    ${err.message}`
        });
    }
};
